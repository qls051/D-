...
public class EnemyFSM : MonoBehaviour
{
    ...
    private EnemyMemoryPool enemyMemoryPool; // 적 메모리풀 (적 오브젝트 비활성화에 사용)
    ...
    private Vector3 CalculateWanderPosition()
    {
        ...
        // 현재 적 캐릭터가 있는 월드의 중심 위치와 각도 ( 구역을 벗어난 행동을 하지 않도록)
        Vector3 rangePosition = Vector3.zero;
        Vector3 rangeScale = Vector3.one * 100.0f;

        // 자신 위치를 중심으로 반지름(wanderRadius) 거리, 선택된 각도(wanderJitter)에 위치한 자표를 목표지점으로 설정
        ...
        targetPosition.x = Mathf.Clamp(targetPosition.z, rangePosition.z - rangeScale.z * 0.5f, rangePosition.z + rangeScale.z * 0.5f);
